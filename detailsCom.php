...
if($id == '' || $token == ''){
    ...
    if($token == $token_tmp){
        $sql = $con->prepare("SELECT count(codProd) FROM productos WHERE codProd = ? AND codigoCat = 3");
        $sql->execute([$id]);
        if($sql->fetchColumn() > 0){
            $sql = $con->prepare("SELECT stockProd, nomProd, precio, imagen FROM productos WHERE codProd = ? AND codigoCat = 3 LIMIT 1");
            $sql->execute([$id]);
